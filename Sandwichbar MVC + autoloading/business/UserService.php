...
class UserService
{
    ...
    public function validateUser(string $user_email, string $user_password): bool
    {
        ...
        if (($user !== null) && ($user->getStatus() !== 'geblokkeerd')) {
            return true;
        } else {
            return false;
        }
    ...
    {
        $string_status = 'actief';

        if (($user->getStatus() == 'actief') && ($user->getType() !== 'medewerker')) {
            $string_status = 'actief <a href="overzicht.php?blokkeer_id=' . $user->getId() . '">(Blokkeren)</a>';
        } elseif (($user->getStatus() == 'geblokkeerd') && ($user->getType() !== 'medewerker')) {
            $string_status = 'geblokkeerd <a href="overzicht.php?activeer_id=' . $user->getId() . '">(Activeren)</a>';
        }
