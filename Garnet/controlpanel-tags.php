...
if ($user->getRole() == 'admin') {
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'add_tag') {
        ...
        if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
            $_SESSION['feedback'] = json_encode(['message' => 'An error has occurred.', 'type' => 'error']);
            header("Location: controlpanel-tags");
            exit(0);
        }
        ...
        $_SESSION['feedback'] = json_encode(['message' => $tag_name . ' added as a primary tag with weight ' . $tag_weight . ' and description "' . $tag_description . '".', 'type' => 'success']);
        header("Location: controlpanel-tags.php");
        exit(0);
    ...
    if (isset($_GET['remove_tag']) && $_GET['remove_tag'] != "") {
        ...
        $_SESSION['feedback'] = json_encode(['message' => 'Tag deleted.', 'type' => 'success']);
        header("Location: controlpanel-tags.php");
        exit(0);
    }
