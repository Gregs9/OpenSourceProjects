...
class UserService
{
    ...
    public function getUserStatusAction(User $user): string
    {
        ...
        if (($user->getStatus() == 'active') && ($user->getRole() !== 'admin')) {
            $string_status = 'active <a title="Click to block this user." class="regular-link" href="controlpanel_users?block_id=' . $user->getId() . '">(Block)</a>';
        } elseif (($user->getStatus() == 'blocked') && ($user->getRole() !== 'admin')) {
            $string_status = 'blocked <a title="Click to reactivate this user." class="regular-link" href="controlpanel_users?activate_id=' . $user->getId() . '">(Activate)</a>';
        }
    ...
    public function getUserRoleAction(User $user): string
    {
        ...
        //prevent myself from getting demoted haha
        if ($user->getUsername() == 'greg') {
            return 'webmaster';
        }

        if (($user->getRole() == 'user') && ($user->getUsername() !== 'admin')) {
            $string_status = 'user <a title="Click to promote this user." class="regular-link" href="controlpanel_users?promote_id=' . $user->getId() . '">(Promote)</a>';
        } elseif (($user->getRole() == 'admin') && ($user->getUsername() !== 'admin')) {
            $string_status = 'admin <a title="Click to demote this user." class="regular-link" href="controlpanel_users?demote_id=' . $user->getId() . '">(Demote)</a>';
        }
        return $string_status;
    }
    ...
    public function getUserDeleteAction(User $user): string
    {
        ...
        //prevent myself from getting deleted haha
        if ($user->getUsername() == 'greg') {
