...
<div id="tag-builder">
    ...
    <div id="available-tags-container">
        ...
        <div id="available-tags">
            <input type="search" id="search-tags" placeholder="Search.." title="Search for a tag">
            <?php foreach ($tagSvc->getAllTags() as $tag) { ?>
                <a title="<?php echo $tag->getDescription() ?>" class="tag"
                    style="display:none"><?php echo $tag->getName() ?></a>
            <?php } ?>
        </div>
    </div>
