    ...
    <div class="wrapper">
        ...
        <main id="overzicht">
            ...
            <div id="lever-info">
                ...
                <p id="full-name">{{Session::get('last_name')}} {{Session::get('first_name')}}</p>
                <p id="address">
                    {{Session::get('adres')}}
                </p>
                <p id="city">
                    {{Location::findOrFail(intval(Session::get('plaatsId')))->postcode}} {{Location::findOrFail(intval(Session::get('plaatsId')))->plaats}} 
                </p>
            ...
            <form id="wijzig-info" style="display:none" method="post" action="{{ route('adresWijzigen') }}">
                ...
                <label for="wijzig_adres">Straat en Huisnr <input name="wijzig_adres" minlength="5" maxlength="45"
                        required type="text" value="{{Session::get('adres')}}"></label>
                <br>
                <label for="wijzig_postcode">Postcode <input name="wijzig_postcode" type="text" minlength="4" required
                        maxlength="4"
                        value="{{Location::findOrFail(intval(Session::get('plaatsId')))->postcode}}"></label>
                <br>
                <label for="wijzig_gemeente">Gemeente <input name="wijzig_gemeente" type="text" minlength="2"
                        maxlength="45" required
                        value="{{Location::findOrFail(intval(Session::get('plaatsId')))->plaats}}"></label>
                <br>
